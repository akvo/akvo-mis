...
| Frontend — pages | Implement `FormBuilderList`, `FormBuilderCreate`, `FormBuilderEdit` (previously empty scaffolds) |
...
- **Editor uses `onSave` not `onChange`**: `WebformEditor` exposes only an `onSave` callback (triggered by its built-in Save button). There is no `onChange` event. Draft saves to localStorage happen inside `onSave` with a 2 s debounce; draft is restored on page mount.
- **CSS imported per-page**: `akvo-react-form-editor/dist/index.css` is imported in `FormBuilderCreate.jsx` and `FormBuilderEdit.jsx`. Webpack deduplicates repeated imports.
